...
void hit_matching(int verbose = 0, int nEvtMx = 0){
	...
	//PDF file
	c1->Print("copper_eic_hybrid/hitmatchingMuons.pdf(");
	for (int ic = 1; ic<nCanvas; ic++) cs[ic]->Print("copper_eic_hybrid/hitmatchingMuons.pdf");
	if (ftof_back_xy){
          for (int ic = 1; ic<nC11s; ic++) c11s[ic]->Print("copper_eic_hybrid/hitmatchingMuons.pdf");
	}
	lastC->Print("HybridSVTOB/hitmatchingEPCorrectedMaterialpdf)");
	//ROOT file
